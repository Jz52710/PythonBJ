...
def wzq_win1(wzq, guige, xuanshou):
    ...
    for i in range(1, guige + 1):
        for j in range(1, guige - 3):
            if (wzq[(i, j)] == wzq[(i, j + 1)] == wzq[(i, j + 2)] == wzq[(i, j + 3)] == wzq[(i, j + 4)] and wzq[
                (i, j)] in ["X", "O"]):
...
def wzq_win2(wzq, guige, xuanshou):
    ...
    for i in range(1, guige - 3):
        for j in range(1, guige + 1):
            if (wzq[(i, j)] == wzq[(i + 1, j)] == wzq[(i + 2, j)] == wzq[(i + 3, j)] == wzq[(i + 4, j)] and wzq[
                (i, j)] in ["X", "O"]):
                print("%s获胜,游戏结束！" % xuanshou)
                exit()
...
def wzq_win3(wzq, guige, xuanshou):
    ...
    for i in range(1, guige - 3):
        for j in range(1, guige - 3):
            if (wzq[(i, j)] == wzq[(i + 1, j + 1)] == wzq[(i + 2, j + 2)] == wzq[(i + 3, j + 3)] == wzq[
                (i + 4, j + 4)] and wzq[(i, j)] in ["X", "O"]):
                print("%s获胜,游戏结束！" % xuanshou)
                exit()
